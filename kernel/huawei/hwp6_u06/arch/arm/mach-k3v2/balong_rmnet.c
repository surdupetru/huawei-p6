...
#define RMNET_NAME   "rmnet""_balong"
#define N_RMNET   N_RMNET_BALONG
/* Rename for compile conflict */
#define g_rmnet_private   g_rmnet_private##_balong
#define rmnet_wq   rmnet_wq##_balong
...
static int rmnet_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    if(p && p->tty && p->tty->ops && p->tty->ops->write){
        ...
        ret = p->tty->ops->write(p->tty, (&skb->data[RMNET_ETH_HDR_SIZE ]), ((skb->len) - RMNET_ETH_HDR_SIZE));
        if(0 >= ret){
...
static int __init rmnet_init(void)
{
    int ret;
    int i;
    char dev_name[128]; //sizeof(RMNET_NAME"%d")+1+1;
    struct net_device *dev;
    struct rmnet_private *p;
    ...
    for (i=0; i<MAX_NET_IF; i++) {
        ...
        snprintf(dev_name, sizeof(dev_name), RMNET_NAME"%d", i);
...
static int __init rmnet_async_init(void)
{
    ...
    err = get_modem_type( MODEM_BALONG );
    pr_info("MODEM[%s] %s: get_modem_type \"%s\". %d\n",MODEM_DEVICE_BOOT(MODEM_BALONG),__func__,MODEM_BALONG,err);
    if( err<=0 ) {
