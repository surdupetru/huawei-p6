 ...
 */
int k3v2_pinmux_func(struct iomux_pin *pin, enum lowlayer_func pin_func)
{
	int value1;
	int i = 0;
	int *func_value = NULL;
...
static int k3v2_iocg_writevalue(struct iomux_pin *pin, enum pull_updown
...
int k3v2_pinmux_setpullupdown(struct iomux_pin *pin,
		enum pull_updown pin_pull_updown)
{
	...
	ret = k3v2_iocg_writevalue(pin, pin_pull_updown, NOINPUT);

	return ret;
}

int k3v2_pinmux_setdrivestrength(struct iomux_pin *pin,
	enum drive_strength pin_drive_strength)
{
	...
	if ((pin_drive_strength != LEVEL0) && (pin_drive_strength != LEVEL1) \
		...
		(pin_drive_strength != LEVEL3) && (pin_drive_strength != LEVEL4)) {
		pr_debug("IOMUX:the drive strength value is invalid.\r\n");
		return -INVALID;
	}

	ret = k3v2_iocg_writevalue(pin, NOINPUT, pin_drive_strength);

	return ret;
}

int k3v2_block_setfunc(struct iomux_block  *blockmux,
	struct block_config *config, enum iomux_func newfunc)
{
	...
	while (*pins_temp && (config[newfunc].func_value[i] != -INVALID) \
		...
		ret = k3v2_pinmux_func(*pins_temp, config[newfunc].func_value[i]);
		if (ret < 0)
			break;
		...
		/*set status of block*/
		ret = k3v2_iocg_writevalue(*pins_temp, config[newfunc].pullud_value[i],\
...
struct iomux_ops iomux_pin_ops = {
	.pin_setfunc = k3v2_pinmux_func,
	.pin_setpullupdown = k3v2_pinmux_setpullupdown,
	.pin_setdrivestrength = k3v2_pinmux_setdrivestrength,
};

struct iomux_ops iomux_block_ops = {
	.block_setfunc = k3v2_block_setfunc,
};
