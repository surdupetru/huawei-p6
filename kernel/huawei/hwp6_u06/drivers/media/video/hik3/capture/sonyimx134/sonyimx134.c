...
#define SONYIMX134_MAX_ISO 			3100
#define SONYIMX134_MIN_ISO                      100

#define SONYIMX134_MIN_CAP_FRAMERATE  8
#define SONYIMX134_FLASH_TRIGGER_GAIN 0xff
#define SONYIMX134_SHARPNESS_PREVIEW  0x0A
#define SONYIMX134_SHARPNESS_CAPTURE  0x0A

#define SONYIMX134_SHARPNESS_LF_P1		0x08
#define SONYIMX134_SHARPNESS_LF_P2		0x20
#define SONYIMX134_SHARPNESS_HGAIN_P		0x04

#define SONYIMX134_FACE_SHARPNESS_LF_P1		0x06
#define SONYIMX134_FACE_SHARPNESS_LF_P2		0x20
#define SONYIMX134_FACE_SHARPNESS_HGAIN_P	0x02
#define SONYIMX134_FACE_SHARPNESS_LGAIN_P	0x08
...
#define SONYIMX134_AUTOFPS_GAIN_LOW2MID		0xE0
#define SONYIMX134_AUTOFPS_GAIN_MID2HIGH		0x48

#define SONYIMX134_AUTOFPS_GAIN_HIGH2MID		0xC0
#define SONYIMX134_AUTOFPS_GAIN_MID2LOW		0x1D0
...
static framesize_s sonyimx134_framesizes[] = {
	...
	//1920x1088,24fps for 1080P video
	//{0, 2, 1920, 1088, 3600, 1858, 24, 10, 0x1bc, 0x172,0x100, VIEW_FULL, RESOLUTION_16_9,false,{sonyimx134_framesize_1920x1088, ARRAY_SIZE(sonyimx134_framesize_1920x1088)} },

	//1920x1088,30fps for 1080P video
	{0, 2, 1920, 1088, 3600, 1858, 30, 10, 0x22b, 0x1CE,0x100, VIEW_CROP, RESOLUTION_16_9,false,{sonyimx134_framesize_1920x1088, ARRAY_SIZE(sonyimx134_framesize_1920x1088)} },
	...
	{0, 2, 3264, 2448, 4150, 2962, 13, 15, 0x181, 0x141, 0xDE, VIEW_FULL, RESOLUTION_4_3, false, {sonyimx134_framesize_full, ARRAY_SIZE(sonyimx134_framesize_full)} },
};
...
static int sonyimx134_init_isp_reg(void)
{
	...
	else
	{
		...
		{
			if(OTPCHECKSUMFLAG ==OTPCHECKSUM_FLAG)//module with new tune otp paremeters
			{
				size = ARRAY_SIZE(isp_init_regs_sonyimx134_new_Liteon);
				sonyimx134_write_isp_seq(isp_init_regs_sonyimx134_new_Liteon, size);
			}
			else
			{
...
static void sonyimx134_switch_to_lowlight_isp_seq(bool mode)
{
	u32 size = sizeof(sonyimx134_lowlight_regs_backup)/sizeof(sonyimx134_lowlight_regs_backup[0]);

	print_debug("enter %s", __func__);
	if(mode == true)
	{
		sonyimx134_read_isp_seq(sonyimx134_lowlight_regs_backup, size);
		if(sensor_module ==  MODULE_SUNNY)
			sonyimx134_write_isp_seq(sonyimx134_lowlight_regs_Sunny, size);
		else
			sonyimx134_write_isp_seq(sonyimx134_lowlight_regs_Liteon, size);
	}
	else
	{
		sonyimx134_write_isp_seq(sonyimx134_lowlight_regs_backup, size);
	}
...
static void sonyimx134_get_face_sharpness_param(u8 *mbuf, bool mode, u8 size)
{
	...
	if(mode){
		mbuf[0] = SONYIMX134_FACE_SHARPNESS_LF_P1;
		mbuf[1] = SONYIMX134_FACE_SHARPNESS_LF_P2;
		mbuf[2] = SONYIMX134_FACE_SHARPNESS_HGAIN_P;
		mbuf[3] = SONYIMX134_FACE_SHARPNESS_LGAIN_P;
	}
	else{
		mbuf[0] = SONYIMX134_SHARPNESS_LF_P1;
		mbuf[1] = SONYIMX134_SHARPNESS_LF_P2;
		mbuf[2] = SONYIMX134_SHARPNESS_HGAIN_P;
		mbuf[3] = SONYIMX134_SHARPNESS_CAPTURE;
	}
...
static u32 sonyimx134_get_override_param(camera_override_type_t type)
{
	...
	case OVERRIDE_FPS_MIDDLE:
		ret_val = SONYIMX134_MIDDLE_FRAMERATE;
		break;
...
static int sonyimx134_fill_uvsaturation_buf( camera_state state)
{
	...
	//u8 awb_CW_R_shift[2];
	//u8 awb_D_R_shift[2];
    uv_satu_buf[0] = SONYIMX134_ISP_UV_MAX_SATURATION;
	uv_satu_buf[1] = SONYIMX134_ISP_UV_MAX_SATURATION;
   ...
   if (state == STATE_PREVIEW)
    {
	...
	#if 0
		awb_A_R_shift[0] = 0x97;
		awb_A_R_shift[1] = 0xA4;
		...
		uv_curve_buf[8] = 0x80;
		uv_curve_buf[9] = 0x80;
		uv_curve_buf[10] = 0x80;
		uv_curve_buf[11] = 0x80;
		uv_curve_buf[12] = 0x80;
		uv_curve_buf[13] = 0x80;
		uv_curve_buf[14] = 0x70;
#endif
	}
   else  if (state == STATE_CAPTURE)
   {
	   ...
	   uv_satu_buf[1] = 0x68;
        ...
        uv_curve_buf[8] = 0x8b;
        uv_curve_buf[9] = 0x8b;
        uv_curve_buf[10] = 0x8b;
        uv_curve_buf[11] = 0x8b;
        uv_curve_buf[12] = 0x8b;
        uv_curve_buf[13] = 0x84;
        uv_curve_buf[14] = 0x73;
#endif

   }
    SETREG8(REG_ISP_UV_MAX_SATURATION, uv_satu_buf[0]);
	SETREG8(REG_ISP_UV_MIN_SATURATION, uv_satu_buf[1]);
#if 0
	SETREG8(REG_ISP_UV_CURVE_1, uv_curve_buf[0]);
	SETREG8(REG_ISP_UV_CURVE_2, uv_curve_buf[1]);
	SETREG8(REG_ISP_UV_CURVE_3, uv_curve_buf[2]);
	...
	SETREG8(REG_ISP_UV_CURVE_11, uv_curve_buf[10]);
	SETREG8(REG_ISP_UV_CURVE_12, uv_curve_buf[11]);
	SETREG8(REG_ISP_UV_CURVE_13, uv_curve_buf[12]);
...
static int sonyimx134_fill_denoise_buf(u8 *ybuf, u16 *uvbuf, u8 size, camera_state state, bool flash_on)
{
	...
	{
	//y denoise
		ybuf[4] = SONYIMX134_ISP_YDENOISE_COFF_8X;
		ybuf[5] = SONYIMX134_ISP_YDENOISE_COFF_8X;
		ybuf[6] = SONYIMX134_ISP_YDENOISE_COFF_8X;
		//uv denoise
		uvbuf[4] = SONYIMX134_ISP_UVDENOISE_COFF_8X;
		uvbuf[5] = SONYIMX134_ISP_UVDENOISE_COFF_8X;
		uvbuf[6] = SONYIMX134_ISP_UVDENOISE_COFF_8X;
	}
	else
	{
		if (flash_on == false)
	...
	//sonyimx134_fill_uvsaturation_buf(state);
...
static int sonyimx134_try_current_hdr_framesizes(struct v4l2_frmsize_discrete *fs)
{
	int i = 0;
	bool match = false;
	int ret = 0;
...
static void sonyimx134_set_default(void)
{
	...
	sonyimx134_sensor.switch_to_lowlight_isp_seq = sonyimx134_switch_to_lowlight_isp_seq;
	/* zhoutian 00195335 2013-03-02 added for SuperZoom-LowLight end > */
	sonyimx134_sensor.get_face_sharpness_param = sonyimx134_get_face_sharpness_param;

	sonyimx134_sensor.support_binning_type	= BINNING_AVERAGE;

	sonyimx134_sensor.try_current_hdr_framesizes =  sonyimx134_try_current_hdr_framesizes;
}
