...
#define CSI0_REG_OFFSET  (0x7F800)
#define CSI1_REG_OFFSET  (0x7FC00)
...
int ispv1_get_resource(void)
{
	...
	/* get reg base */
	res = platform_get_resource_byname(ispv1_io.pdev, IORESOURCE_MEM, "isp_base");
	if (res == NULL) {
	...
	ispv1_io.csi0_base = IO_ADDRESS(res->start) + CSI0_REG_OFFSET;
	ispv1_io.csi1_base = IO_ADDRESS(res->start) + CSI1_REG_OFFSET;
	...
	clk = clk_get(NULL, "clk_dphy1");

	if (IS_ERR_OR_NULL(clk)) {
		print_error("failed to get phy clock");
		ret = -ENXIO;
		goto fail;
	}
...
int ispv1_io_power_init(char *ldo_name, u32 vol_min, u32 vol_max)
{
	...
	strncpy(ispv1_io.ldo[ispv1_io.ldo_count].name, ldo_name,
		strlen(ldo_name) + 1);
...
int ispv1_io_power_sensor(camera_power_state power_state, char *ldo_name)
{
	...
	print_info("%s config LDO: ldo_count=%d power_state=%d",
		   __func__, ispv1_io.ldo_count, power_state);
	...
	if (power_state == POWER_ON) {
		...
		for (i = 0; i < ispv1_io.ldo_count; i++) {
			if (0 == strcmp(ldo_name, ispv1_io.ldo[i].name)) {
				regulator_disable(ispv1_io.ldo[i].regulator);
				regulator_put(ispv1_io.ldo[i].regulator);
				ispv1_io.ldo[i].regulator = NULL;
			}
		}
