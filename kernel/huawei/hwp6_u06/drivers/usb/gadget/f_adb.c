...
static int adb_sleep_thread(struct adb_dev *dev)
{
	int	rc = 0;
	...
	for (;;) {
		...
		if (signal_pending(dev->thread_task)) {
			rc = -EINTR;
			break;
		}
		if (dev->thread_wakeup_needed)
			break;
		schedule();
	}
...
static void adb_wakeup_thread(struct adb_dev *dev)
{
...
static int adb_open(struct inode *ip, struct file *fp)
{
	...
	_adb_dev->error = 0;
	_adb_dev->thread_wakeup_needed = 0;
