...
#ifdef SOFTAP
static int
init_ap_profile_max_scb_from_string(char *param_str, struct ap_profile *ap_cfg)
{
        char *str_ptr = param_str;
        char sub_cmd[16];
        int ret = 0;
...
static int
set_ap_cfg_max_scb(struct net_device *dev, struct ap_profile *ap)
{
            ...
            char *ext)
{
        ...
        if (strstr(fw_path, "_apsta") == NULL) {
                WL_ERROR(("Can't execute %s(), SOFTAP fw is not Loaded. %s!!!\n",
        ...
        if (wrqu->data.length != 0) {
                ...
                if ((res = init_ap_profile_max_scb_from_string(extra, ap_cfg)) < 0) {
        ...
        if ((res = set_ap_cfg_max_scb(dev, ap_cfg)) < 0)
