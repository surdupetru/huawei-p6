...
static void lsm330_gyr_input_work_func(struct work_struct *work)
{
	...
	if (err < 0)
		...
		lsm330_gyr_report_values(stat, &data_out);
	if(atomic_read(&stat->enabled))
		schedule_delayed_work(&stat->input_work, msecs_to_jiffies(10));  //stat->pdata->poll_interval
	mutex_unlock(&stat->lock);
}
...
static int lsm330_gyr_input_init(struct lsm330_gyr_status *stat)
{
	...
	if (err) {
		...
		err = set_sensor_input(GYRO, stat->input_dev->dev.kobj.name);
		if (err) {
...
int set_selftest_lm330(int value)
{
	...
	TEST_RESULT:
		if(result == SELF_TEST_OK){
			dev_info(&stat_temp->client->dev, "gyro self test ok---------\n");
			err = set_gyro_selfTest_result(GYRO, "1");
			if (err) {
				dev_err(&stat_temp->client->dev, " gyro set self test result  fail\n");
			}
		}else {
			...
			dev_err(&stat_temp->client->dev, "gyro self test fail\n");
		}
		msleep(50);
		...
		dev_info(&stat_temp->client->dev, "selftest_gyro-------out\n");
		return SELF_TEST_OK;
	}
