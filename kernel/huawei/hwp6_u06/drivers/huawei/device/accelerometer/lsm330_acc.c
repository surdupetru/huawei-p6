					...
					int *xyz)
{
	...
	if (1 == calibration_value)
	{
		...
		else
		{
			...
			temp[2] = (1024 - accel[2] /10)/SCALE;
			/*****judge whether offset<200mg********/
			if(temp[0]>100||temp[0]<-100
					||temp[1]>100||temp[1]<-100
						||temp[2]>100||temp[2]<-100)
			{
	...
	input_report_abs(acc->input_dev, ABS_Z, accl_data[2] + gsensor_offset[2] * SCALE);
...
static int read_gsensor_offset_from_nv()
{
	...
	strcpy(user_info.nv_name, "gsensor");
	...
	if (gsensor_offset[2] > OFFSET_BIG)
		gsensor_offset[2] = gsensor_offset[2] - 256;
...
static int lsm330_acc_later_resume(struct early_suspend *h)
{
	...
	//printk("-----------lsm330acc later resume begin-----\n");
...
{
	struct lsm330_acc_data *acc = container_of(h, struct lsm330_acc_data, early_suspend);
	//printk("-----------lsm330 early suspend begin-----\n");
