...
#define PEN_SENS_LOW        146				//low sensitivity
...
u8  touch_state = 0;							//0-0 state, 1-finger state, 2-pen state
u8  pen_release_flag = 0;
u16  finger_edge_stat = 0;
					...
					u8 finger_state, u8 n_finger,  struct rmi_device *rmi_dev)
{
	...
	finger_edge_stat &= ~(1 << n_finger);
	/*set a certain finger bit to 0*/
	...
	if (((finger_edge_stat & (1 << n_finger)) == 0) && n_finger < 16) {
		/*previous finger with the same id up and judge whether abandon the finger at edge*/
		if (x > XMINF && x < XMAXF)
			finger_edge_stat |= (1 << n_finger);
			/*if at display area, set finger with the same id bit to 1*/
