...
/*touchkey water proof add begin*/
#include "rmi_f01.h"
/*touchkey water proof add end*/
...
extern int i2c_synaptics_write(uint16_t address, uint16_t data);
		...
		*/
		...
		switch(touch_axis_align_value){
			...
			case E_FLIP_X_FALSE_Y_TRUE:
				axis_align->flip_y = ~ (axis_align->flip_y);
				break;
			case E_FLIP_X_TRUE_Y_FALSE:
				axis_align->flip_x = ~ (axis_align->flip_x);
				break;
...
static int rmi_f11_register_devices(struct rmi_function_container *fc)
{
	...
	dev_dbg(&fc->dev, "touchkey_sensitivity = %d\n", f11->touchkey_sensitivity);
	...
	f11->jdi_tk_enable = get_jdi_tk_enable();//if has touchkey
...
static int rmi_f11_set_tk_sensitivity(struct rmi_function_container *fc)
{
	...
	if (touchkey_sensitivity == 0) {
		...
		if ((atomic_read(&tp_last_status) == TP_FINGER) && (glove_mode == TP_GLOVE)) {
			i2c_synaptics_write(SENSOR_PIN10_SENSITIVITY, DEFAULT10_SENSITIVITY_GLOVE);
			i2c_synaptics_write(SENSOR_PIN32_SENSITIVITY, DEFAULT32_SENSITIVITY_GLOVE);
			atomic_set(&tp_last_status, TP_GLOVE) ;
		} else if ((atomic_read(&tp_last_status) == TP_GLOVE) && (glove_mode == TP_FINGER)) {
			i2c_synaptics_write(SENSOR_PIN10_SENSITIVITY, DEFAULT10_SENSITIVITY);
			i2c_synaptics_write(SENSOR_PIN32_SENSITIVITY, DEFAULT32_SENSITIVITY);
            ...
            if (device_status.wet == 1) {
                if ((((struct f11_data *)fc->data)->water_mode == DRY_MODE)
                    ...
                    i2c_synaptics_write(SENSOR_PIN10_SENSITIVITY, DEFAULT10_WET);
                    i2c_synaptics_write(SENSOR_PIN32_SENSITIVITY, DEFAULT32_WET);
                ...
                if ((((struct f11_data *)fc->data)->water_mode == WET_MODE)
                    ...
                    if (((struct f11_data *)fc->data)->water_cnt == 0) {
                        ...
                        i2c_synaptics_write(SENSOR_PIN10_SENSITIVITY, DEFAULT10_DRY);
                        i2c_synaptics_write(SENSOR_PIN32_SENSITIVITY, DEFAULT32_DRY);
