	...
	const struct i2c_device_id *i2c_id)
{
    ...
    if (get_cyttsp4_enable() == false){
        dev_info(&client->dev, "%s: cyttsp4_i2c_adapter disable\n", __func__);
        return  -EINVAL;
    }
    ...
    /* VDD power on */
    // CYPRESS_VDD "ts-vdd"
	vdd = regulator_get(&client->dev, "ts-vdd-cypress");
	if (IS_ERR(vdd)) {
		dev_err(&client->dev, "%s: failed to get cypress vdd\n", __func__);
		return  -EINVAL;
	}

	error = regulator_set_voltage(vdd,2850000,2850000);
	if(error < 0){
	...
	/* VBUS power on */
    // CYPRESS_VBUS
	vbus = regulator_get(&client->dev, "ts-vbus-cypress");
	if (IS_ERR(vbus)) {
