...
static struct wlcore_conf wl18xx_conf = {
	...
	.conn = {
		...
		.synch_fail_thold            = 36,//12,
		.bss_lose_timeout            = 1000,//400,
...
static int wl18xx_set_clk(struct wl1271 *wl)
{
	...
	if (wl18xx_clk_table[clk_freq].swallow) {
        ...
        /* choose WCS PLL */
        ret = wl18xx_top_reg_write(wl, PLLSH_WL_PLL_SEL, 0x0);
        if (ret < 0)
                goto out;
...
#include "../../../../../../../include/hsad/config_interface.h"
#define WL18XX_CONF_FILE_NAME "ti-connectivity/wl18xx-conf/wl18xx-conf.bin"
#define WIFI_FW_NAME_LEN 25
#define FULL_FILE_NAME_LEN 80
...
static int wl18xx_conf_init(struct wl1271 *wl, struct device *dev)
{
	...
	int ret;
    char tmp[WIFI_FW_NAME_LEN];
    char file_name[FULL_FILE_NAME_LEN];
    
    memset(tmp, 0, sizeof(tmp));
    if(get_wifi_wl18xx_rf_fw_name(tmp))
    {
        sprintf(file_name, "ti-connectivity/wl18xx-conf/wl18xx-conf-%s.bin", tmp);
        printk("load conf file:%s\n", file_name);
...
static int __devinit wl18xx_probe(struct platform_device *pdev)
{
	...
	hw = wlcore_alloc_hw(sizeof(struct wl18xx_priv),
			     WL18XX_AGGR_BUFFER_SIZE, WL18XX_NUM_TX_DESCRIPTORS);
	if (IS_ERR(hw)) {
	...
	if (ret){
		wl1271_error("wlcore_probe return err:%d", ret);
		goto out_free;
	}
	return ret;
