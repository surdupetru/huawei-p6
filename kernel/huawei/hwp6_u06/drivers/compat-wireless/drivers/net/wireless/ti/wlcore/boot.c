					     ...
					     size_t fw_data_len, u32 dest)
{
	...
	if (ret < 0){
		wl1271_error("wifi bug test: wl1271_boot_upload_firmware_chunk  "
			"wlcore_set_partition error. partition.mem.start=%d, err:%d", 
				partition.mem.start, ret);
		goto out;
	}
	...
	while (chunk_num < fw_data_len / CHUNK_SIZE) {
		...
		if (addr > partition_limit) {
			...
			ret = wlcore_set_partition(wl, &partition);
			if (ret < 0){
				wl1271_error("wifi bug test:  addr > partition_limit  "
					"wlcore_set_partition error. partition.mem.start=%d, err:%d", 
						partition.mem.start, ret);
				goto out;
			}
	...
	if (ret < 0){
		wl1271_error("wifi bug test: wl1271_boot_upload_firmware_chunk  "
		"last chunk wlcore_write  error num=%d", ret);
	}
out:
	kfree(chunk);
	return ret;
