...
/*K3V2_WIFI_PM_ERR_FLAG_CLEAN*/
extern int k3v2_set_wifi_card_dev_ptr(struct device *  dev_ptr);
				  ...
				  const struct sdio_device_id *id)
{
	...
	/*K3V2_WIFI_PM_ERR_FLAG_CLEAN*/
	/*record the wifi card dev pointer.*/
	k3v2_set_wifi_card_dev_ptr(&func->card->dev);
	k3v2_clean_card_pm_error_flag();
	/* We are only able to handle the wlan function */
	if (func->num != 0x02)
		return -ENODEV;
	printk(KERN_INFO "wifi info: wl1271_probe enter");
